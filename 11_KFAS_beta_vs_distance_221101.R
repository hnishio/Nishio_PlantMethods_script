...
out <- "11_KFAS_beta_vs_distance_221101"
...
glist <- lm_dist_beta(cell_list = cell_list, mvtime = mvtime, 
                      ssm_path = "08_KFAS_chloroplast_velocity_221101",
                      ssm_method = "KFAS", res_name = "chloroplast",
                      ex_name = "microbeam", unit1 = "micrometer", unit2 = "min")

g <- glist[[1]] + glist[[2]] + glist[[3]] + 
  plot_layout(nrow = 1)
suppressWarnings(ggsave(paste0(out, "/KFAS_lm_dist_beta.pdf"),
                        g, height = 104/2, width = 168, units = "mm"))
